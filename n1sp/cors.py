...
class CorsMiddleware(object):
    ...
    def process_request(self, request):
        ...
        """
        if (self.is_enabled(
                request) and request.method == 'OPTIONS' and 'HTTP_ACCESS_CONTROL_REQUEST_METHOD' in request.META):
    ...
    def process_response(self, request, response):
        ...
        if self.is_enabled(request) and origin:
            ...
            response[ACCESS_CONTROL_ALLOW_ORIGIN] = "*" if (
                settings.CORS_ORIGIN_ALLOW_ALL and not settings.CORS_ALLOW_CREDENTIALS) else origin
            ...
            if request.method == 'OPTIONS':
                ...
                if settings.CORS_PREFLIGHT_MAX_AGE:
                    response[
                        ACCESS_CONTROL_MAX_AGE] = settings.CORS_PREFLIGHT_MAX_AGE
